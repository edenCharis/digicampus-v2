...
import type { Etablissement, Cycle, ApiList } from '../_shared'
import {
...
export default function CyclesPage() {
  ...
  const load = useCallback(() => {
    if (!etabId) return
    apiFetch<ApiList<Cycle>>(`/cycles/?etablissement=${etabId}&limit=200`)
  ...
  function openEdit(c: Cycle) {
    setEditing(c)
    setForm({ code: c.code, libelle: c.libelle })
    setErr(null); setOpen(true)
  }
  ...
  async function save() {
    ...
    try {
      ...
      if (!editing) await apiFetch('/cycles/', { method: 'POST', body: JSON.stringify(body) })
      else await apiFetch(`/cycles/${editing.id}/`, { method: 'PATCH', body: JSON.stringify(body) })
      setOpen(false); load()
    } catch (e: unknown) {
  ...
  async function doDelete() {
    if (!del) return
    await apiFetch(`/cycles/${del.id}/`, { method: 'DELETE' }).catch(() => {})
    setDel(null); load()
  }

  const etabLabel = etabs.find(e => String(e.id) === etabId)?.libelle || ''
  function accent(c: Cycle) { return COLORS[c.id % COLORS.length] }
    ...
    <>
      ...
      <div className="pg-header">
        <div>
          ...
          <p className="pg-sub">{etabLabel && `${etabLabel} · `}{filtered.length} cycle{filtered.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      <div className="pt-wrap">
        <div className="pt-toolbar">
          <div className="pt-toolbar-left">
            <SearchInput value={search} onChange={v => { setSearch(v); setPage(1) }} placeholder="Rechercher un cycle…" />
          </div>
        </div>
        ...
        <table className="pt-table">
          ...
          <tbody>
            {paged.length === 0 && (
              <tr><td colSpan={3}><div className="pt-empty"><RotateCcw size={28} style={{ margin: '0 auto 8px', opacity: 0.3 }} /><div>Aucun cycle</div></div></td></tr>
            )}
            {paged.map(c => (
              <tr key={c.id}>
                <td>
                  <div className="pt-cell-row">
                    <div className="pt-avatar" style={{ background: accent(c) }}>
                      {c.code.slice(0, 2).toUpperCase()}
                    </div>
                    <span className="pt-primary">{c.libelle}</span>
                  </div>
                </td>
                <td><span className="pt-badge" style={{ background: `${accent(c)}18`, color: accent(c) }}>{c.code}</span></td>
                <td>
                  <div className="pt-actions">
                    <button className="pt-ico-btn" onClick={() => openEdit(c)} title="Modifier"><Pencil size={12} /></button>
                    <button className="pt-ico-btn del" onClick={() => setDel(c)} title="Supprimer"><Trash2 size={12} /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {open && (
        <Modal onClose={() => setOpen(false)}>
          <ModalHead title={editing ? 'Modifier le cycle' : 'Nouveau cycle'} onClose={() => setOpen(false)} />
          <ModalBody>
            {err && <ErrBanner msg={err} />}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.875rem' }}>
              <FLabel label="Libellé" req>
                <FInput value={form.libelle} onChange={e => setForm(f => ({ ...f, libelle: e.target.value }))} placeholder="Ex: Licence" />
              </FLabel>
              <FLabel label="Code" req>
                <FInput value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value.toUpperCase() }))} placeholder="Ex: LIC" />
              </FLabel>
            </div>
          </ModalBody>
          <ModalFoot>
