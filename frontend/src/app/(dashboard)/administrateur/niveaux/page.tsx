...
import type { Etablissement, Niveau, ApiList } from '../_shared'
import {
...
export default function NiveauxPage() {
  ...
  const [list, setList]         = useState<Niveau[]>([])
  const [search, setSearch]     = useState('')
  const [page, setPage]         = useState(1)
  const [open, setOpen]         = useState(false)
  const [editing, setEditing]   = useState<Niveau | null>(null)
  const [del, setDel]           = useState<Niveau | null>(null)
  ...
  const load = useCallback(() => {
    if (!etabId) return
    apiFetch<ApiList<Niveau>>(`/niveaux/?etablissement=${etabId}&limit=200`)
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return list
    return list.filter(n => n.libelle.toLowerCase().includes(q) || n.code.toLowerCase().includes(q))
  }, [list, search])
  ...
  function openEdit(n: Niveau) {
    setEditing(n)
    setForm({ code: n.code, libelle: n.libelle, ordre: String(n.ordre) })
    setErr(null); setOpen(true)
  }
  ...
  async function save() {
    ...
    try {
      ...
      if (!editing) await apiFetch('/niveaux/', { method: 'POST', body: JSON.stringify(body) })
      else await apiFetch(`/niveaux/${editing.id}/`, { method: 'PATCH', body: JSON.stringify(body) })
      setOpen(false); load()
    } catch (e: unknown) {
  ...
  async function doDelete() {
    if (!del) return
    await apiFetch(`/niveaux/${del.id}/`, { method: 'DELETE' }).catch(() => {})
    setDel(null); load()
  }

  const etabLabel = etabs.find(e => String(e.id) === etabId)?.libelle || ''
  function accent(n: Niveau) { return COLORS[n.id % COLORS.length] }
  ...
  return (
    <>
      ...
      <div className="pg-header">
        <div>
          <h1 className="pg-title">Niveaux</h1>
          <p className="pg-sub">{etabLabel && `${etabLabel} · `}{filtered.length} niveau{filtered.length !== 1 ? 'x' : ''}</p>
        </div>
      ...
      <div className="pt-wrap">
        <div className="pt-toolbar">
          <div className="pt-toolbar-left">
            <SearchInput value={search} onChange={v => { setSearch(v); setPage(1) }} placeholder="Rechercher un niveau…" />
          </div>
        </div>
        ...
        <table className="pt-table">
          ...
          <tbody>
            {paged.length === 0 && (
              <tr><td colSpan={4}><div className="pt-empty"><Layers size={28} style={{ margin: '0 auto 8px', opacity: 0.3 }} /><div>Aucun niveau</div></div></td></tr>
            )}
            {paged.map(n => (
              <tr key={n.id}>
                <td>
                  <div className="pt-cell-row">
                    <div className="pt-avatar" style={{ background: accent(n) }}>
                      {n.code.slice(0, 2).toUpperCase()}
                    </div>
                    <span className="pt-primary">{n.libelle}</span>
                  </div>
                </td>
                <td><span className="pt-badge" style={{ background: `${accent(n)}18`, color: accent(n) }}>{n.code}</span></td>
                <td style={{ color: '#64748b', fontSize: '0.8125rem' }}>{n.ordre}</td>
                <td>
                  <div className="pt-actions">
                    <button className="pt-ico-btn" onClick={() => openEdit(n)} title="Modifier"><Pencil size={12} /></button>
                    <button className="pt-ico-btn del" onClick={() => setDel(n)} title="Supprimer"><Trash2 size={12} /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {open && (
        <Modal onClose={() => setOpen(false)}>
          <ModalHead title={editing ? 'Modifier le niveau' : 'Nouveau niveau'} onClose={() => setOpen(false)} />
          <ModalBody>
            {err && <ErrBanner msg={err} />}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.875rem' }}>
              <FLabel label="Libellé" req>
                <FInput value={form.libelle} onChange={e => setForm(f => ({ ...f, libelle: e.target.value }))} placeholder="Ex: Licence 1" />
              </FLabel>
              <FLabel label="Code" req>
                <FInput value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value.toUpperCase() }))} placeholder="Ex: L1" />
              </FLabel>
            </div>
