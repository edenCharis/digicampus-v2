...
import { Plus, Pencil, Trash2, GitFork, Search, X } from 'lucide-react'
import { apiFetch } from '@/lib/api'

const PAGE_SIZE = 20

interface Parcours { id: number; code: string; libelle: string; etablissement: number; etablissement_libelle: string }
...
export default function ParcoursPage() {
  ...
  const [delTarget, setDelTarget] = useState<Parcours | null>(null)
  ...
  async function handleSave(ev: React.FormEvent) {
    ...
    try {
      ...
      if (editing) await apiFetch(`/parcours/${editing.id}/`, { method: 'PATCH', body: JSON.stringify(body) })
      else await apiFetch('/parcours/', { method: 'POST', body: JSON.stringify(body) })
      setOpen(false); load()
    } catch (e: unknown) {
  ...
  async function handleDelete() {
    if (!delTarget) return
    await apiFetch(`/parcours/${delTarget.id}/`, { method: 'DELETE' }).catch(console.error)
    ...
    <>
      ...
      <div className="pg-header">
        <div>
          ...
          <p className="pg-sub">{loading ? '…' : `${filtered.length} parcours`}</p>
        </div>
      ...
      <div className="sc-wrap">
        <div className="sc-toolbar">
          <div className="sc-search">
            ...
            <input placeholder="Rechercher un parcours…" value={search}
        ...
        <div style={{ overflowX: 'auto' }}>
          <table className="sc-table">
            <thead>
              <tr><th>Parcours</th><th>Code</th><th></th></tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={3} className="sc-empty">
                  <GitFork size={32} color="#e2e8f0" style={{ margin: '0 auto .5rem', display: 'block' }} />
                  Aucun parcours
                </td></tr>
              ) : paged.map(p => (
                <tr key={p.id}>
                  <td>
                    <div className="sc-row">
                      <div className="sc-avatar" style={{ background: accent(p.id) }}>
                        {p.code.slice(0, 2).toUpperCase()}
                      </div>
                      <span style={{ fontWeight: 600, color: '#0f172a' }}>{p.libelle}</span>
                    </div>
                  </td>
                  <td>
                    <span className="sc-badge" style={{ background: `${accent(p.id)}1a`, color: accent(p.id) }}>{p.code}</span>
                  </td>
                  <td>
                    <div className="sc-actions">
                      <button className="sc-btn" onClick={() => openEdit(p)}><Pencil size={14} /></button>
                      <button className="sc-btn del" onClick={() => setDelTarget(p)}><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        {totalPages > 1 && (
          <div className="sc-footer">
            <span className="sc-count">{filtered.length} parcours · Page {page}/{totalPages}</span>
        ...
        <div className="mo" onClick={e => e.target === e.currentTarget && setOpen(false)}>
          <div className="mo-box">
            <div className="mo-head">
              <span className="mo-title">{editing ? 'Modifier le parcours' : 'Nouveau parcours'}</span>
              ...
              <div className="mo-body">
                ...
                <div className="fl-grid2">
                  <div className="fl">
                    ...
                    <input required value={form.libelle} onChange={e => setForm({ ...form, libelle: e.target.value })} placeholder="Sciences Informatiques" />
                  </div>
                  <div className="fl">
                    <label>Code <span>*</span></label>
                    <input required value={form.code} onChange={e => setForm({ ...form, code: e.target.value.toUpperCase() })} placeholder="INFO" />
                  </div>
                </div>
              </div>
      ...
      {delTarget && (
        <div className="mo" onClick={e => e.target === e.currentTarget && setDelTarget(null)}>
          <div className="mo-box" style={{ maxWidth: 380 }}>
            <div className="mo-head">
              <span className="mo-title">Supprimer le parcours</span>
              <button className="mo-x" onClick={() => setDelTarget(null)}><X size={16} /></button>
            </div>
            <div className="mo-body">
              <p style={{ fontSize: '.875rem', color: '#475569' }}>
                Supprimer le parcours <strong>{delTarget.libelle}</strong> ? Cette action est irréversible.
              </p>
            </div>
            <div className="mo-foot">
